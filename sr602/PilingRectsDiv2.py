...
class PilingRectsDiv2:
    def getmax(self, X, Y, limit):
        limitX = math.sqrt(limit)
        if not limitX == int(limitX):
            limitX = limitX + 1
        n = len(X)
        ret = 0        
        for i in range(1, int(limitX)+1):
            maxR = 0
            for j in range(n):
                tmpX = min(X[j], Y[j])
                tmpY = max(X[j], Y[j])
                if tmpX >= i and tmpY >= float(limit)/float(i):
                    maxR = maxR + 1
            ret = max(ret, maxR)
        if ret == 0:
            return -1
